...
export default function JobDetailPage() {
  ...
  async function addTask() {
    ...
    const { data, error: err } = await supabase
      ...
      .insert({
        ...
        job_id: captureJobId ?? jobId,
        surface_date: surfaceDate,
        location_text: captureLocation.trim().length > 0
          ? captureLocation.trim()
          : (job?.location_text ?? null),
        lat: captureLocationCoords?.lat ?? job?.lat ?? null,
        lng: captureLocationCoords?.lng ?? job?.lng ?? null,
      })
      .select()
      .single();
    if (err) {
  ...
  const nextNeedsToday =
    nextTask != null && isScheduledForLater(nextTask, todayStr);
        ...
        <>
          ...
          {!done && tasks.length > 0 && (
            ...
            >
              <div
                ...
                style={{ margin: 0, display: 'flex', flexWrap: 'wrap', gap: '6px 12px' }}
              >
                {todayCount > 0 && (
          ...
          {tasks.length === 0 ? (
            <div className="empty-state">
              ...
              <button className="btn btn-steel" onClick={openCapture}>
                Add a task
              </button>
            </div>
          ) : (
