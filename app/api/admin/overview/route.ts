...
async function buildOverview(): Promise<OverviewPayload> {
  ...
  for (const u of sortedSignups.slice(0, 6)) {
    if (!u.created_at) continue;
    recentAccounts.push({ id: u.id, email: maskEmail(u.email ?? null), createdAt: u.created_at });
  }
  ...
  const recentChanges: { id: string; email: string | null; status: string; updatedAt: string }[] = agg.accounts.recentChanges.map(
    (c) => ({ id: c.user_id, email: maskEmail(emailById.get(c.user_id) ?? null), status: c.status, updatedAt: c.updated_at })
  );
  ...
  const feedbackRecent: OverviewPayload['feedback']['recent'] = agg.feedback.recent.map((f) => ({
    id: f.id,
    label: f.is_anonymous ? 'Anonymous' : maskEmail(f.submitter_email),
    message: f.message,
  ...
  const feedbackEvents = agg.feedback.recent.slice(0, 3).map((f) => ({
    ...
    title: f.is_anonymous ? 'Feedback received (anonymous)' : `Feedback received from ${maskEmail(f.submitter_email)}`,
