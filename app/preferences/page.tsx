...
export default function Preferences() {
  ...
  async function enableNotifications() {
    try {
      ...
      // Fast path: already blocked — requestPermission() will not show a dialog.
      ...
      // Ensure SW is registered before FCM/token work (can hang on .ready otherwise).
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(
                  'Service worker took too long. Refresh the page and try again.'
                )
              ),
            15000
          )
