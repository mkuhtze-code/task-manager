...
export default function JobFilesPanel(props: {
  ...
  const { jobId, userId, variant = 'job', defaultCollapsed = false } = props;
  ...
  const dropProps =
    isDesktop && !sectionCollapsed ? capture.bindDropTarget((m) => void addCaptured(m)) : {};
        ...
        <>
          {variant === 'meeting' && (
            <p style={{ fontSize: 12, color: 'var(--ink-soft)', margin: '0 0 8px', lineHeight: 1.4 }}>
              Stored on this job. Observations stay on the meeting.
            </p>
          )}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
            ...
            <button
              type="button"
              className={
                activeFolderId === UNFILED ? 'meeting-pill meeting-pill--primary' : 'meeting-pill'
              }
              onClick={() => {
